...
contract xWinFarm is IBEP20, BEP20 {
    ...
    modifier onlyManager {
     ...
     constructor (
            ...
            bscswapRouter = IBSCswapRouter02(address(0xd954551853F55deb4Ae31407c423e67B1621424A));
    ...
    internal returns (uint amountToken, uint amountBNB, uint liquidity) {
        ...
        TransferHelper.safeApprove(farmToken, address(bscswapRouter), amount); 
...
/// Get All the fund data needed for client
    ...
    function _getWithdrawRewardWithCushion(address tokenaddress, uint256 withdrawQty) internal view returns ( 
        ...
        pair = BSCswapLibrary.pairFor(bscswapRouter.factory(), tokenaddress, bscswapRouter.WBNB());
        totalSupply = IBEP20(pair).totalSupply(); 
        ratio = withdrawQty.mul(1e18).div(totalSupply);
        
        if(ratio > 0){
            ( reserveA,  reserveB) = BSCswapLibrary.getReserves(bscswapRouter.factory(), tokenaddress,  bscswapRouter.WBNB());
            ATokenAmount = reserveA.mul(ratio).div(1e18);
            amountB = BSCswapLibrary.quote(ATokenAmount, reserveA, reserveB);
        }else{
            ATokenAmount = 0;
            amountB = 0;
        }
    ...
    function _getPairBalance(address _targetAdd) internal view returns (uint, uint) {
        ...
        address pair = BSCswapLibrary.pairFor(bscswapRouter.factory(), _targetAdd, bscswapRouter.WBNB());
        ...
        (uint reserveA,  uint reserveB) = BSCswapLibrary.getReserves(bscswapRouter.factory(), _targetAdd,  bscswapRouter.WBNB());
        
        //convert A to BNb
        uint myPortions = reserveA.mul(ratio).div(1e18);
        uint AtoBNBEstAmount = BSCswapLibrary.quote(myPortions, reserveA, reserveB);
    ...
    function _getQuotes(
        ...
        (uint reserveA, uint reserveB) = BSCswapLibrary.getReserves(bscswapRouter.factory(), bscswapRouter.WBNB(), targetToken);
        amountOut = BSCswapLibrary.getAmountOut(bnbQty, reserveA, reserveB);
        amountB = BSCswapLibrary.quote(bnbQty, reserveA, reserveB);
    ...
    function _removeFromLP(uint256 redeemUnit, uint256 deadline) 
        ...
        TransferHelper.safeApprove(pair, address(bscswapRouter), redeemUnit); 
        (amountToken, amountBNB) = bscswapRouter.removeLiquidityBNB(
            farmToken,
            redeemUnit,
            ATokenAmountMin,
            amountBMin, 
     ...
     function _getFarmCakeEstBalance() 
        ...
        if(farmTokenBalance > 0){
            (uint reserveA,  uint reserveB) = BSCswapLibrary.getReserves(bscswapRouter.factory(), farmToken, bscswapRouter.WBNB());
            farmtoBNBEstAmount = BSCswapLibrary.quote(farmTokenBalance, reserveA, reserveB);
        }
        if(totalCakeBal > 0){
            (uint cakereserveA,  uint cakereserveB) = BSCswapLibrary.getReserves(bscswapRouter.factory(), cakeToken, bscswapRouter.WBNB());
            caketoBNBEstAmount = BSCswapLibrary.quote(totalCakeBal, cakereserveA, cakereserveB);
        }
    ...
    function emergencyRedeem(uint256 redeemUnit, address _investorAddress) external onlyxWinProtocol payable {
        ...
        address pair = BSCswapLibrary.pairFor(bscswapRouter.factory(), farmToken, bscswapRouter.WBNB());
