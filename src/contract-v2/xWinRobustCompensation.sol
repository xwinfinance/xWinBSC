...
contract xWINRobustCompensation is ReentrancyGuard, Ownable {
    ...
    */
    ...
    constructor() public {
        name = "xWIN CompensationWithdraw";
    }
    ...
    function withdrawCompensation()  public payable nonReentrant {
        
        require(blackList[msg.sender] == false, "the user has no compensate balance"); 

        require(hasWithdrawed[msg.sender] == false, "already withdrawed");

        uint256 compensateBalance =  _xWinCompensation.viewMyCompensation(msg.sender);

        require(compensateBalance > 0, "no compensate balance");

        require(_xWinCompensation.releaseBlock() < block.number, "not yet release");
    ...
    function addUserToBlackList(address _user) public onlyOwner {
        blackList[_user] = true;  // is banned user
    }

    function removeUserFromBlackList(address _user) public onlyOwner {
