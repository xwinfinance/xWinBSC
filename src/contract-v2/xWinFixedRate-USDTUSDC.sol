...
contract xWINUSDTStaking is ReentrancyGuard, IBEP20, BEP20 {
    ...
    bool public swapAuto = true; 
    bool public swapUSDC = true; 
    ...
    address public usdtusdcLP = address(0xEc6557348085Aa57C72514D67070dC863C0a5A8c);
    address public xwin = address(0xd88ca08d8eec1E9E09562213Ae83A7853ebB5d28);
    address public burnAddress = address(0x000000000000000000000000000000000000dEaD);
    address public platformWallet = address(0xaD8234bC07168d9f7E49c3780e4b5fefa6620f8A);
    ...
    function updateTokenAddresses(
        ...
        usdtusdcLP = _usdtusdcLP;
        xwin = _xwin;
    }

    function updateSwapFlag(bool _swapAuto, bool _swapUSDC) public onlyOwner {
        swapAuto = _swapAuto;
        swapUSDC = _swapUSDC;
    }
    ...
    function _swapUSDCToUSDT() internal {
        ...
        uint256 usdcBalance = IBEP20(usdc).balanceOf(address(this));
        address[] memory path = new address[](2);
        path[0] = usdc;
        path[1] = usdt;
        _swapTokenToToken(usdcBalance, usdc, address(this), path);
    }
    
    function swapXWINToUSDT(uint _amount) public onlyOwner {
    ...
    function stakeToken(uint _pid, uint _amount) external nonReentrant payable {
        ...
        require(IBEP20(usdt).balanceOf(msg.sender) >= _amount, "Not enough balance");
        ...
        uint nextStart = block.number.add(1);
        user.farmingStart = nextStart; //
        user.lastHarvest = nextStart;
        uint endDay = pool.lockDays.mul(oneDayBlock);
        user.lockedEnd = nextStart.add(endDay);
        ...
        _exchangeDepositLP(finalDeposit);
        _mint(address(this), finalDeposit);
        pool.totalLockedSupply = pool.totalLockedSupply.add(_amount);
    ...
    function unStakeToken(uint _pid) external nonReentrant payable {
        ...
        PoolInfo storage pool = poolInfo[_pid];
        require(pool.maxLimit > 0, "No pool found");
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.lockedEnd <= block.number, "still in locked period");
        
        //remove LP from PCS
        uint withdrawAmt = getOwnershipStakeAmount(user.amount);
        ...
        uint burnFee = xwinrewards.mul(250).div(10000);
        TransferHelper.safeTransfer(xwin, burnAddress, burnFee); 
        TransferHelper.safeTransfer(xwin, msg.sender, xwinrewards.sub(burnFee));

        //swap back to usdt
        if(swapUSDC) _swapUSDCToUSDT();
        ...
        TransferHelper.safeTransfer(usdt, msg.sender, totalReturnToUser); 
    ...
    function emergencyRemove() public onlyOwner {
    ...
    function _exchangeDepositLP(uint _amount) internal returns (uint liquidity) {
        ...
        path[0] = usdt;
        path[1] = usdc;
        uint outputSwap = _swapTokenToToken(halfAmt, usdt, address(this), path);
        ...
        TransferHelper.safeApprove(usdtusdcLP, address(_autoFarm), liquidity); 
    ...
    function _removeFromLP(uint _amount) internal {
        ...
        uint LPtotalSupply = IBEP20(usdtusdcLP).totalSupply(); 
        uint ratio = _amount.mul(1e18).div(LPtotalSupply);
        uint amountA = 0;
        uint amountB = 0;
        
        (uint  reserveA,  uint reserveB) = PancakeLibrary.getReserves(pancakeSwapRouter.factory(), usdt, usdc);
        ...
        TransferHelper.safeApprove(usdtusdcLP, address(pancakeSwapRouter), _amount); 
    ...
    function _addLiquidity(uint usdcAmount, uint usdtAmount) internal returns (uint amountA_, uint amountB_, uint liquidity) {
        
        TransferHelper.safeApprove(usdc, address(pancakeSwapRouter), usdcAmount); 
        TransferHelper.safeApprove(usdt, address(pancakeSwapRouter), usdtAmount); 
        
        uint usdtMin = usdtAmount.sub(usdtAmount.mul(slippage).div(10000));
        uint usdcMin = usdcAmount.sub(usdcAmount.mul(slippage).div(10000));
